...
It is ***your*** responsability to make your scripts reproducible. If I take your script 4 years later, I should be able to run it.
...
If I give the folder "good-practices" to someone else:

* the output of `here::here()` will change because the location of the folder on the computer changed.

* my code will still run because inside the folder because the path to the data didn't change.

## Relative paths in R

<br>

Another good thing: `{here}` will also work in you open the script outside of an RStudio project, and will work on all operating systems (e.g no paths problems because of Windows or Mac).
...
If you already use `rm(list = ls())` at the beginning of your script, good job!

<br>

{{< fa arrow-right >}} &nbsp; **you're wrong!**
...
* `rm()`: remove some objects from the environment
...
As you can see, it still works, meaning that the package was not unloaded.
...
Instead of putting `rm(list = ls())` at the beginning of the script, you should 
completely restart the RStudio session to be sure your code can run in a fresh session:
